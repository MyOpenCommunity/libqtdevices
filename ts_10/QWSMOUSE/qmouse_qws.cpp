...
{
#if 1 // MODIFIED
	// The data stored in the calibration file si calculated using the algoritms
	// of tslib in order to reduce the noise of the touchscreen.
	...
	QPoint dev_bl = data->devPoints[QWSPointerCalibrationData::BottomLeft];
	QPoint screen_bl = data->screenPoints[QWSPointerCalibrationData::BottomLeft];
	...
	QPoint dev_tr = data->devPoints[QWSPointerCalibrationData::TopRight];
	QPoint screen_tr = data->screenPoints[QWSPointerCalibrationData::TopRight];

	QPoint dev_center = data->devPoints[QWSPointerCalibrationData::Center];
	QPoint screen_center = data->screenPoints[QWSPointerCalibrationData::Center];

	struct
	{
		int x[5]; /* read from device */
		int xfb[5]; /* crosspoint coordinates */;
		int y[5];
		int yfb[5];
		int a[7];
	} cal;

	/* Copying to tslib structure */
	cal.x[0] = dev_tl.x();
	cal.y[0] = dev_tl.y();
	cal.xfb[0] = screen_tl.x();
	cal.yfb[0] = screen_tl.y();

	cal.x[1] = dev_bl.x();
	cal.y[1] = dev_bl.y();
	cal.xfb[1] = screen_bl.x();
	cal.yfb[1] = screen_bl.y();

	cal.x[2] = dev_br.x();
	cal.y[2] = dev_br.y();
	cal.xfb[2] = screen_br.x();
	cal.yfb[2] = screen_br.y();

	cal.x[3] = dev_tr.x();
	cal.y[3] = dev_tr.y();
	cal.xfb[3] = screen_tr.x();
	cal.yfb[3] = screen_tr.y();

	cal.x[4] = dev_center.x();
	cal.y[4] = dev_center.y();
	cal.xfb[4] = screen_center.x();
	cal.yfb[4] = screen_center.y();
	...
	float det = n * (x2*y2 - xy*xy) + x * (xy*y - x*y2) + y * (x*xy - y*x2);
	if (det < 0.1 && det > -0.1)
		qWarning() << "QWSCalibratedMouseHandler::calibrate: determinant is too small";
	...
	/* STRANGE COEFFICIENT RE_ORDERING */
	a = static_cast<int>(cal.a[1]);
	b = static_cast<int>(cal.a[2]);
	c = static_cast<int>(cal.a[0]);
	e = static_cast<int>(cal.a[5]);
	d = static_cast<int>(cal.a[4]);
	f = static_cast<int>(cal.a[3]);
