...
{
	sendFrame(createDimensionFrame(who, QString("#%1#%2*%3").arg(_DIM_STATE_UPDATE_INTERVAL).arg(mode).arg(UPDATE_INTERVAL), where));
}

void EnergyDevice::sendUpdateStop()
{
	sendFrame(createDimensionFrame(who, QString("#%1#%2*%3").arg(_DIM_STATE_UPDATE_INTERVAL).arg(mode).arg(0), where));
}
