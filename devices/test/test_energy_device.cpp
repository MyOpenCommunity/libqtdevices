...
void TestEnergyDevice::sendRequestCumulativeMonth()
{
	...
	QCOMPARE(server->frameCommand(), QString("*#18*20*53##"));
}
...
void TestEnergyDevice::sendRequestCumulativeMonth2()
{
	...
	client_command->flush();
	QCOMPARE(server->frameCommand(), QString("*#18*20*52#9#1##"));
}
...
void TestEnergyDevice::sendRequestCumulativeYearGraph()
{
	...
	QCOMPARE(server->frameCommand(), frames.join(""));
}
...
void TestEnergyDevice::receiveCumulativeDay2()
{
	...
	int day = QDate::currentDate().day() == 1 ? 2 : 1;
	QString tmp(QString("*#18*%1*%2#8#%3").arg(where).arg(EnergyDevice::DIM_DAY_GRAPH).arg(day));
	QStringList frames;
	...
	t.check(frames, EnergyValue(QDate(year, 8, day), 52500));
}
...
void TestEnergyDevice::receiveCumulativeDay16Bit()
{
	...
	DeviceTester t(dev, EnergyDevice::DIM_CUMULATIVE_DAY, DeviceTester::MULTIPLE_VALUES);
	int day = QDate::currentDate().day() == 1 ? 2 : 1;
	QString tmp(QString("*#18*%1*%2#8#%3").arg(where).arg(511).arg(day));
	QStringList frames;
	...
	t.check(frames, EnergyValue(QDate(year, 8, day), 17477));
}
...
void TestEnergyDevice::receiveCumulativeMonth()
{
	...
	t.check(QString("*#18*%1*52#10#4*4294967294##").arg(where), EnergyValue(QDate(2010, 4, 1), 4294967294u));
}
...
{
	DeviceTester t(dev, EnergyDevice::DIM_ADVANCED_DEVICE);

	t.checkSignals(QString("*#18*%1*1200#%2*2##").arg(where).arg(upd->mode), 1);
...
void TestEnergyDevice::receiveUpdateStop()
{
	...
	t.checkSignals(QString("*#18*%1*1200#%2*0##").arg(where).arg(upd->mode), 1);
	QCOMPARE(dev->has_new_frames, true);
	QCOMPARE(upd->has_new_frames, true);
	QCOMPARE(upd->update_timer, (QTimer *)NULL);
}
...
void TestEnergyDevice::testUpdateStartPolling()
{
	...
	QCOMPARE(server->frameCommand(), reqr + reqc);
