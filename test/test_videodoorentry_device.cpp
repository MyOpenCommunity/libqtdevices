...
void TestVideoDoorEntryDevice::receiveIncomingCall()
{
	...
	t << makePair(VideoDoorEntryDevice::RINGTONE, (int)VideoDoorEntryDevice::PE1);
...
void TestVideoDoorEntryDevice::receiveFloorCall()
{
	...
	QString frame = QString("*8*1#%1#%2#21*%3##").arg(kind).arg(mmtype).arg(dev->where);
	t.check(frame, (int)VideoDoorEntryDevice::FLOORCALL);
	QCOMPARE(dev->kind, 1);
...
void TestVideoDoorEntryDevice::receivePagerCall()
{
	...
	t << makePair(VideoDoorEntryDevice::RINGTONE, (int)VideoDoorEntryDevice::PI_INTERCOM);
...
void TestVideoDoorEntryDevice::receivePagerCallWithWhere()
{
	...
	t2 << makePair(VideoDoorEntryDevice::RINGTONE, (int)VideoDoorEntryDevice::PI_INTERCOM);
	QString frame2 = QString("*8*1#%1#%2#%3*%4##").arg(kind).arg(mmtype).arg("15").arg("11");
	t2.check(frame2);
	QCOMPARE(dev->kind, 14);
	QCOMPARE(dev->mmtype, 2);
}
...
void TestVideoDoorEntryDevice::receivePagerCallWithWrongWhere()
{
	...
	DeviceTester t(dev, VideoDoorEntryDevice::END_OF_CALL);
	QString frame = QString("*8*1#%1#%2#%3*%4##").arg(kind).arg(mmtype).arg("16").arg("22");
	t.checkSignals(frame, 0);
}
